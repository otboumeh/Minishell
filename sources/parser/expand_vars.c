...
int	expand_var(int index, t_mini *m)
{
	...
	if (m->tokens[index][0] == '\'' && \
	m->tokens[index][ft_strlen(m->tokens[index]) - 1] == '\'')
		return (1);
...
int	exchange_token_expansion(char *tkn, t_mini *m)
{
	...
	{
		if (m->str_var_name[i] == '\0' || \
		(9 <= m->str_var_name[i] && m->str_var_name[i] <= 13) || \
...
int	exchange_token_expansion_aux_aux(char *tkn, t_mini *m)
{
	...
	if (!m->str_pre_var_name)
		return (m_exit("Couldn't alloc in \
		exchange_token_expansion_aux_aux", m), 0);
